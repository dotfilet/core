...
import_git() {
  ...
  if [[ -d "${cache_dir}" ]]; then
    ...
    if (( needs_stash )); then git stash --include-untracked; fi
...
use() {
  ...
  local filet_module_path=$(resolve_module "${module}")
  eval "FILET_MODULE_ROOT_${variable_name}=${filet_module_path:h}"

  evaluate_script "${filet_module_path}"
}
